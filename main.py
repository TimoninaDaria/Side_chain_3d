...
class PDBS_alignment:
    ...
    def check_if_alignment_is_ok(self):
        ...
        for i in list_of_bad_backbone_aa:
            print('Warning: PDB entry {} contains incomplete set of backbone atoms for amino acid(s)'.format(self.dirs_in_fasta_alignment_and_in_pdb_folder[i]), list_of_bad_backbone_aa[i])
        ...
        for i in range(len(self.alignment)):
            for j in range(i, len(self.alignment)):
                ...
                if len(rmsds) != 0 :
                    ...
                    if mean_rmsd > 9:
                         print('Warning: Average RMSD between {} and {} is {} what may indicate poor alignment quality of the two proteins'.format(self.dirs_in_fasta_alignment_and_in_pdb_folder[i], self.dirs_in_fasta_alignment_and_in_pdb_folder[j],str(mean_rmsd)))
...
class One_column_clustering:
    ...
    def printing_py_result(self, output_folder):
        for i in range(len(self.lab)):
            f.write('cmd.show("sticks", "{} and resi {}")\n'.format(self.mols_and_aa[i][0].base_filename, self.mols_and_aa[i][1]))
            if self.lab[i] == 0:
                ...
                f.write('cmd.color("{}", "{} and resi {}")\n'.format(self.lab[i] + 3, self.mols_and_aa[i][0].base_filename, self.mols_and_aa[i][1]))
            else:
                f.write('cmd.color("{}", "{} and resi {}")\n'.format(self.lab[i] + 3, self.mols_and_aa[i][0].base_filename, self.mols_and_aa[i][1]))
...
The latest version of the program, documentation and examples are available open-access at https://biokinet.belozersky.msu.ru/Zebra3D
    ...
    def input_preparation(self):
        ...
        if 'cpu_threads' in self.dict_of_sys_argv:
            self.cpu_threads = int(self.dict_of_sys_argv['cpu_threads'])
        else:
            self.cpu_threads = multiprocessing.cpu_count()

        if 'method' in self.dict_of_sys_argv:
            assert self.dict_of_sys_argv['method'] in ['optics', 'dbscan', 'hdbscan'], 'Method must be optics, dbscan or hdbscan'
            self.method_of_clustering = self.dict_of_sys_argv['method']
        else:
            self.method_of_clustering = 'hdbscan'
        ...
        if 'min_cluster_size' in self.dict_of_sys_argv:
            self.min_cluster_size = int(self.dict_of_sys_argv['min_cluster_size'])
        else:
            self.min_cluster_size = 5
        
        if 'min_samples' in self.dict_of_sys_argv:
            self.min_samples = int(self.dict_of_sys_argv['min_samples'])
        else:
            self.min_samples = 5    
        
        if 'max_content_of_gaps' in self.dict_of_sys_argv:
            self.perc_of_gaps = float(self.dict_of_sys_argv['max_content_of_gaps'])/100.
        else:
           self.perc_of_gaps = 0.05 

        if 'max_content_of_mismatch' in self.dict_of_sys_argv:
            self.max_content_of_mismatch = float(self.dict_of_sys_argv['max_content_of_mismatch'])/100.
        else:
            self.max_content_of_mismatch = 0.05
        ...
        if 'max_outliers' in self.dict_of_sys_argv:
            self.max_outliers = float((self.dict_of_sys_argv['max_outliers']))/100.
        else:
            self.max_outliers = 1.0
...
if __name__ == "__main__":
    ...
    for heap in tqdm(heaps):
        ...
        cl.clustering(min_cluster_size=input.min_cluster_size, min_samples=input.min_samples, eps=input.eps_for_clustering, cpu_threads=input.cpu_threads)
        cls.append(cl)

f = open(os.path.join('out', 'RESULT.py'), 'w')
f.write('cmd.bg_color("white")\n')
for i in range(len(alignment.molecules)):
    f.write('cmd.load(r"{}", "{}")\n'.format(alignment.molecules[i].filename, alignment.molecules[i].base_filename))
    f.write('cmd.color("silver", "{}")\n'.format(alignment.molecules[i].base_filename))        

cls_sorted = sorted(cls, key=lambda x: x.score if x.score is not None else -9999, reverse=True)
for cl in cls_sorted[0:10]:
    if cl.score is not None:
        cl.printing_py_result(input.output) 
...
table = [["Rank", "Silhouette_score", "Diameter", "Score"]]
for i, cl in enumerate(cls_sorted[0:10]):
    if cl.score is not None:
        table.append([i + 1, round(cl.sil, 3), round(cl.diam, 3), round(cl.score, 3)])

with open(os.path.join(input.output, 'RESULTS.txt'), 'w') as f:
